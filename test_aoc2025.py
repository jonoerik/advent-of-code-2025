...
def pytest_generate_tests(metafunc):
    if metafunc.function == test:  # pylint: disable=comparison-with-callable
        ...
        for day_dir in sorted(top_dir.iterdir()):
            if day_match := day_regex.fullmatch(day_dir.name):
                ...
                test_inputs = sorted([p for p in data_dir.iterdir()
                                      if sample_regex.fullmatch(p.name)]) + \
                    [data_dir / "input"]
                for test_input in test_inputs:
                    for part in [1, 2]:
                        pytest_generate_single_test(tests, day, test_input, part)

        arguments = tests.items()
        metafunc.parametrize(["day", "input_path", "part1", "answer", "extra_args"],
                             [arg[1] for arg in arguments],
            ids=[arg[0] for arg in arguments])


def pytest_generate_single_test(tests, day, test_input, part):
    ...
    if answer_file.exists():
        ...
        with open(answer_file, encoding="utf-8") as f:
            for answer_line in f.readlines():
                ...
                if ": " in answer_line:
                    extra_args = {extra_arg.split("=")[0]: try_convert_int(extra_arg.split("=")[1])
                                  for extra_arg in answer_line.split(": ")[0].split(",")}
                    answer = answer_line.split(": ")[1]
                else:
                    extra_args = {}
                    answer = answer_line
                answer = try_convert_int(answer)
                tests[f"day{day:02} part{part}: {input_name}" +
                      (" " + str(extra_args) if extra_args else "")] = \
                    (day, test_input, part == 1, answer, extra_args)


def test(day: int, input_path: Path, part1: bool,
         answer: int | str, extra_args: dict[str, int | str]) -> None:
